...
#[tokio::test]
async fn main() {
    let _re = message_flow::connection::connect("localhost:4222".into(), registers!(User))
        .await
        .unwrap();
    let usr = User {
        first_name: "HO".into(),
...
impl User {
    #[message(pattern = "finalay_can")]
    async fn greeting(&self) -> Result<String> {
        Ok("OK HEEELO FROM GREETING".into())
    }
}

// #[msg(pattern = "ee")] async fn gr(&self) -> Result<String> { Ok("OK HEEELO FROM gr".into()) }

// #[event(pattern = "once")] async fn gr(&self) -> Result<String> { Ok("OK HEEELO FROM gr".into())
// }
//
